...
import "../../assets/css/RegisterCost.css";
...
const RegisterCost = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (hasEmptyFields(revenueList) || hasEmptyFields(expenseList)) {
      alert("채워지지 않은 필드가 있습니다. 모든 필드를 채워주세요.");
      return;
    }

    // isLocked가 false인 항목들만 필터링하여 리스트 생성
    ...
    // revenueList와 expenseList를 합친 리스트 생성
    const combinedList = [...unlockedRevenueList, ...unlockedExpenseList].map(
      (item) => ({
        golfCourseId: 100, // golfCourseId 추가
        transactionDate: item.transactionDate,
        transactionType: item.transactionType,
        transactionCategory: item.transactionCategory,
        description: item.description,
        amount: parseInt(item.amount, 10), // amount를 정수로 변환
      })
    );
    console.log(combinedList);

    try {
      // API 호출하여 combinedList를 POST
